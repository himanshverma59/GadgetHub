...
<%@page import="in.gadgethub.dao.impl.*,in.gadgethub.dao.*,javax.servlet.ServletOutputStream,java.io.*,in.gadgethub.pojo.*,java.util.*" %>
   ...
   <tbody> 
    <% 
             ...
             for(CartPojo item:cartItems){
                 ...
                 double currentAmount=prodQuantity*prodPrice;
    ...
    <tr> 
     ...
     <td><a 
      href="CartDetailsServlet?add=1&uid=<%=userName%>&pid=<%=product.getProdId()%>&avail=<%=product.getQuantity()%>&qty=<%=prodQuantity%>"><i 
       class="fa fa-plus"></i></a></td> 
     <td><a 
      href="CartDetailsServlet?add=0&uid=<%=userName%>&pid=<%=product.getProdId()%>&avail=<%=product.getQuantity()%>&qty=<%=prodQuantity%>"><i 
